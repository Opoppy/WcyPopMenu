...
+ (void)createPopMenuItems:(NSArray *)items
    ...
    WcyPopMenuView *menu = [[WcyPopMenuView alloc]initWithItems:items];
...
- (void)calculatingItems {
    ...
    for (NSMutableDictionary *dict in _items) {
        ...
        WcyPopMenuItem *button = [self allocButtonIndex:index With:fromValue];
...
-(void)dismissCompletionBlock:(void(^) (BOOL complete)) completionBlock{
    ...
    for (NSMutableDictionary *dic in _items) {
        WcyPopMenuItem *button = (WcyPopMenuItem *)[self viewWithTag:(index + 1) + kBasePopMenuTag];
        button.attrDic = dic;
 ...
 *  @return PopMenuItem
 */
-(WcyPopMenuItem *)allocButtonIndex:(NSInteger)index With:(CGRect)frame
{
    WcyPopMenuItem *button = [[WcyPopMenuItem alloc] initWithFrame:frame];
 ...
 */
-(void)menuItemSelected:(WcyPopMenuItem *)menuItem
{
    NSInteger tag = menuItem.tag - (kBasePopMenuTag + 1);
    
    __block WcyPopMenuView *weakSelf = self;
    
    for (NSMutableDictionary *dict in _items) {
        NSInteger index = [_items indexOfObject:dict];
        WcyPopMenuItem *buttons = (WcyPopMenuItem *)[self viewWithTag:(index + 1) + kBasePopMenuTag];
        if (index == tag) {
...
-(UILabel *)tipsLbl
{
    if (_tipsLbl == nil)
    {
        _tipsLbl = [[UILabel alloc]initWithFrame:CGRectMake(Width / 9, Height - kBottomViewHeight - kPopMenuItemHeight - 10, Width * 7 / 9, kPopMenuItemHeight)];
...
-(UIImageView *)exitView
{
    if (_exitView == nil)
    {
        ...
        CGPoint center = CGPointMake(Width/2, Height - kBottomViewHeight / 2);
